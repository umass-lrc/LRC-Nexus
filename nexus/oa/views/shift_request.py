...
def shift_requests(request):
    context = {
        ...
        'state': State,
        'change_url_name': 'oa_change_request',
        'drop_url_name': 'oa_drop_request',
        'add_url_name': 'oa_add_request',
    }
...
def add_request(request, request_state):
    requests = ChangeRequest.objects.filter(
        Q(state = request_state) & (Q(position__position = PositionChoices.OFFICE_ASSISTANT) | Q(position__position = PositionChoices.OFFICE_ASSISTANT_PM)),
    ).order_by('start')
    context = {
        'a_requests': requests,
        'url_name': 'oa_add_request_form',
    }
...
def change_request(request, request_state):
    requests = ChangeRequest.objects.filter(
        Q(state = request_state) & (Q(shift__position__position = PositionChoices.OFFICE_ASSISTANT) | Q(shift__position__position = PositionChoices.OFFICE_ASSISTANT_PM)),
    ).order_by('start')
    context = {
        'c_requests': requests,
        'url_name': 'oa_change_request_form',
    }
